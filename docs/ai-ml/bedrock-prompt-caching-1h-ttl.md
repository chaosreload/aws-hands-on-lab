...
- **无缓存**：每次请求全量发送，不设 cache point
- **5min TTL**：系统前缀设 5 分钟缓存（默认 TTL）
- **1h TTL**：系统前缀设 1 小时缓存（新功能）
...
| Turn | 时间 | 间隔 | 任务 | 无缓存 | 5min | 5min 状态 | 1h | 1h 状态 |
|------|------|------|------|--------|--------|--------|--------|--------|
...
| Phase | 场景 | 无缓存 | 5min TTL | 1h TTL | 5min 节省 | 1h 节省 | 1h vs 5min |
|-------|------|---------|--------|------|--------|--------|--------|
