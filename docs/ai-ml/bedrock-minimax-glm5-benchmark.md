...
| **定价（us-east-1）** | $0.30 / $1.20 per 1M tokens | 待官网确认 |
...
| 指标 | MiniMax M2.5 | GLM 5 | 差异 |
|------|:-----------:|:-----:|:----:|
| 简单问答 Input | 51 tokens | 18 tokens | **GLM 2.8x 更省** |
| 简单问答 Output | 99 tokens | 2 tokens | **GLM 49x 更省** |
| 复杂任务 Output | 3,504 tokens | 926 tokens | **GLM 3.8x 更省** |
...
| GLM 5 API 调用 | ~4,500 output tokens × TBD | < $0.01 |
| **合计** | | **< $0.05** |
...
| **成本敏感应用** | GLM 5 | 相同任务 token 消耗低 3-4x |
...
1. **M2.5 的推理 tokens 要纳入成本计算** — 不要只看单价便宜，output tokens 可能是 GLM 5 的 4x
