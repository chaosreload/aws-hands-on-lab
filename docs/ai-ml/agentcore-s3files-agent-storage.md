...
结论先说：**方案 3 在 AgentCore microVM 中不可行**，但方案 1+2 的混合架构是目前最佳实践。
...
|------|----------------|--------|-------------|
| 容量 | 1GB（不可调） | 无限 | 无限 |
| 延迟（1KB 写） | **0.89ms** | 63ms | N/A |
| 延迟（1KB 读） | **0.27ms** | 33ms | N/A |
| 文件系统语义 | ✅ 完整 POSIX | ❌ 对象 API | ✅ NFS 4.2 |
| 跨 session 共享 | ❌ 隔离 | ✅ | ✅ |
| Stop/Resume 持久 | ✅ | ✅ | ✅ |
| AgentCore 可用性 | ✅ 内置 | ✅ boto3 | ❌ 不可行 |
...
!!! warning "S3 Files NFS 挂载在 AgentCore 中不可行"
    这不是权限问题（microVM 有 CAP_SYS_ADMIN），而是 S3 Files 架构限制。S3 Files 强制要求 TLS + IAM 认证的 NFS 挂载，需要 `amazon-efs-utils` mount helper 生态系统，而 AgentCore microVM 环境不支持这一完整流程。
...
| T5 | S3 Files NFS 挂载 | ❌ 不可行 | access denied | 需要 efs-utils + IAM 认证 |
...
| 数据分析 Agent（>1GB 数据集） | Session Storage(热) + S3 API(冷) | 热数据快 230 倍，冷数据容量无限 |
| 多 Agent 协作（需共享文件） | S3 API | Session storage 跨 session 隔离 |
...
│                                  │
│  ┌────────────────────────────┐  │
│  │  /mnt/workspace (1GB)      │  │ ← 热数据：当前工作文件、缓存
│  │  Session Storage            │  │    延迟: <1ms (read), <3ms (write)
│  └────────────────────────────┘  │
│            ↕ boto3                │
│  ┌────────────────────────────┐  │
│  │  S3 Bucket (∞)             │  │ ← 冷数据：历史文件、大数据集、共享
...
### 关于 S3 Files：等待 AgentCore 原生支持

S3 Files NFS 挂载在 AgentCore microVM 中目前不可行，但这可能是暂时的：

- AgentCore microVM **已有** CAP_SYS_ADMIN 能力
- S3 Files **已在** us-west-2 可用
- 如果 AWS 未来在 AgentCore 中原生集成 S3 Files（类似 Lambda 的 EFS 集成），Agent 就能直接获得无限文件系统
- 关注 `filesystemConfigurations` API 的后续更新
