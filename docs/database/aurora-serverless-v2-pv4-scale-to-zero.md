...
- Aurora PostgreSQL 16.3+ / Aurora MySQL 3.08.0+（scale-to-0 engine 版本要求，已核实官方文档）
...
!!! warning "陷阱 1：EC2 launch 脚本里的 `sleep & terminate` 会被 cloud-init 吃掉"
    ...
    **正确做法**：用 `at 'now + 180 min'` 或 `systemd-run --on-active=3h`，别靠 shell 子进程。这是另一起 $1300 空转事故的教训（详见我们 workspace SOUL.md）。
...
**注意**：Paused 状态下 Aurora **不收 instance/compute 费**（[官方确认](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/aurora-serverless-v2-auto-pause.html)："You aren't charged for instance capacity while an instance is in the paused state"）。这是 PV4 scale-to-0 的**真正价值**——不是"秒级唤醒"，而是**真正意义上的按零付费**。
