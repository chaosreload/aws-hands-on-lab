...
### 大数据量验证：134GB Full Load（超越旧 100GB 限制）

为进一步验证存储自动扩展在大数据量下的表现，我们使用更大规模的实例进行了补测：

**测试环境**：

- Source: db.r6g.large, 300GB gp3
- Target: db.r6g.large, 400GB gp3
- DMS Serverless: 1-32 DCU

**数据规模**：

| 表名 | 行数 | 行大小 | 表大小 |
|------|------|--------|--------|
| lob_1 ~ lob_6 | 各 210,000 行 | 100KB LONGTEXT | 各 ~22GB |
| **总计** | **1,260,000 行** | | **~134GB** |

**Full Load 结果**：

| 指标 | 值 |
|------|-----|
| 数据总量 | 134GB（6 表 × 210K 行 × 100KB） |
| Full Load 耗时 | 51 分钟 |
...
**CDC 验证**：Full Load 完成后，在 Source 插入测试行，15 秒内同步到 Target，确认 CDC 正常工作。
...
| 指标 | 基础测试 Full Load | 基础测试 CDC LOB | 134GB Full Load |
|------|---------------|-------------|---------|
| CapacityUtilization | 5.6% | 5.7% | ~4% |
| CPUUtilization | 2.5% | 4-5% | ~7% |
| CDCLatencyTarget | N/A | 峰值 267s | < 15s |
| FullLoadThroughputBandwidthTarget | N/A | N/A | 32MB/s avg |

### 存储扩展行为

**关键发现：存储扩展完全透明，即使在 134GB 数据量下**

- `describe-replications` 的 `ProvisionData` 中**无存储字段**
- CloudWatch 中**无显式存储指标**（如 FreeableStorage、StorageUsed）
- 存储扩展在后台自动进行，用户不可见
- 基础测试 2.3GB CDC 数据传输 + 大规模测试 134GB Full Load 均**无存储相关报错**
- 134GB 远超旧版 100GB 限制，证实自动扩展功能有效
...
!!! warning "踩坑 3：计算资源直接扩展到 Max"
    **现象**：设置 MinCapacityUnits=1, MaxCapacityUnits=16，但启动后直接 provision 了 16 DCU。134GB 测试中 MaxCapacityUnits=32，同样直接扩到 32。
    
    **已查文档确认**：DMS Serverless 根据 metadata 分析预测需要的容量，可能直接分配 Max 值。实际使用率仅 ~4-5.7%，但计费按 provisioned DCU 算。
...
!!! note "大数据量补测费用"
    134GB 补测使用 db.r6g.large 实例（~$0.26/hr）和 32 DCU（~$2.18/hr），运行约 2 小时，额外费用约 ~$5.40。
