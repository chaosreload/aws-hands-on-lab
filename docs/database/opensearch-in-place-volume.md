...
| 资源 | 单价 | 用量 | 费用 |
|------|------|------|------|
| t3.small.search x2 | $0.036/hr | ~2 hr | ~$0.14 |
| r6g.2xlarge.search x2 | $0.578/hr | ~2 hr | ~$2.31 |
| gp3 存储（峰值 ~6.2TB） | $0.08/GB-mo | 短时间 | ~$0.50 |
| **合计（Round 1+2）** | | | **~$3** |

!!! note "费用说明"
    如果要完成逐步扩容的完整 cooldown 等待（6h x 2 步），r6g.2xlarge 实例费用会额外增加约 $7。总计 ~$10。
